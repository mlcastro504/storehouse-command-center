  ...
  if (error) {
    return (
      <div className="text-center text-red-500 py-8">
        Error al cargar el historial de tareas
      </div>
    );
  }
  ...
  if (!tasks || tasks.length === 0) {
    return (
      <div className="text-center text-muted-foreground py-8">
        ...
        <p className="text-sm">
          {showOnlyMine ? 'No has completado tareas aún' : 'No hay tareas completadas en el sistema'}
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div className="text-sm text-muted-foreground">
          {tasks.length} tareas en el historial
        </div>
        <Button
          variant="outline"
          size="sm"
        ...
        >
          ...
          {showOnlyMine ? 'Ver Todas' : 'Solo Mis Tareas'}
        </Button>
      </div>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {task.completed_at
                      ? format(new Date(task.completed_at), 'dd/MM/yyyy HH:mm', { locale: es })
                      : 'N/A'}
                  </p>
                </div>
              </div>
